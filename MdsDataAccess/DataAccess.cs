...
namespace MdsDataAccess
{
    public class DataAccess
    {
        ...
        public dynamic GetDataForTimeRange(DateTime startTime, DateTime endTime)
        {
            ...
            var redisDb = _redis.Value.GetDatabase();
            var dataForTimeRange = new List<IDictionary<string, Tuple<int, int, int, int, int, int>>>();
            while (startTime < endTime)
            {
                ...
                {
                    dataForTimeRange.Add(JsonConvert.DeserializeObject<Dictionary<string, Tuple<int, int, int, int, int, int>>>(retrievedValue));
                }
